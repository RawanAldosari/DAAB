...
class _AddRequestFormState extends State<PostRequestForm> {
  ...
  String? type, club_name, club_location, clubID;
  ...
  final List<String> items = <String>['فعالية', 'متطلبات النادي'];
  ...
  Widget _buildDuration() {
    return TextFormField(
      ...
      controller: duration,
      onSaved: (_val) {
        if (_val != null) duration.text = _val;
      },
      onChanged: (value) {
        setState(
              () {
  ...
  Widget _buildSponsor() {
    return TextFormField(
      ...
      controller: sponsor,
      onSaved: (_val) {
        if (_val != null) sponsor.text = _val;
      },
      onChanged: (value) {
        setState(
              () {
  ...
  Widget _buildStartTime() {
    return TextFormField(
      ...
      decoration: InputDecoration(
        prefixIcon:
        Icon(Icons.watch_later, color:  colors.main, size: 20),
...
// https://medium.com/multiverse-software/alert-dialog-and-confirmation-dialog-in-flutter-8d8c160f4095
      ...
      child: Form(
        ...
        child: Column(
          children: [
            ...
            if (type == "فعالية" && selectedValue == 1)
              Container(
                  width: portrait == true ? 250.w : 300.w,
                  child: Directionality(
                    textDirection: TextDirection.rtl,
            ...
            if (type == "فعالية" && selectedValue == 2)
              Container(
                  width: portrait == true ? 250.w : 300.w,
                  child: Directionality(
                    textDirection: TextDirection.rtl,
  ...
  void add(String? id) async {
    ...
    if (document.exists) {
      ...
      requestVM.duration = duration.text;
      ...
      if (type == "فعالية") {
        ...
        requestVM.sponsor = sponsor.text;
        await requestVM.addEvent();
      }
      else {
        await requestVM.addSupport();
      ...
      Navigator.pushAndRemoveUntil((context),
          MaterialPageRoute(builder: (context) => cHome()), (route) => false);

      _formKey.currentState?.reset();
    } else {
      Snackbar snackbar2 = Snackbar(context, "لا يمكن اضافة الأعلان", "fail");
      snackbar2.showToast();
    }
  }
}
